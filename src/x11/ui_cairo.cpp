...
void renderSettings(cairo_t* cr, const UiState& st) {
    ...
    useFont(cr, g_fonts.mono, 10);
    setc(cr, 102, 102, 102);
    drawText(cr, "tapzap.app", W / 2, 508, 0, 1);
}
...
int liveWindow(double i0, double b0, bool e0) {
    if (ipcSend("show")) return 0; // single instance: a running copy will raise itself
    Display* dpy = XOpenDisplay(nullptr);
    if (!dpy) { std::fprintf(stderr, "cannot open X display (is $DISPLAY set?)\n"); return 2; }
    const int scr = DefaultScreen(dpy);
    // 1x = 320x540 px, matching the macOS popover's 320x540 points on a standard-DPI display.
    // HiDPI users can bump it with TAPZAP_SCALE=2.
    int scale = 1;
    if (const char* s = std::getenv("TAPZAP_SCALE")) { int v = std::atoi(s); if (v >= 1 && v <= 3) scale = v; }
    const int pxW = (int)W * scale, pxH = (int)H * scale;
    int popoverX = 0, popoverY = 0;
    Window win = XCreateSimpleWindow(dpy, RootWindow(dpy, scr), 0, 0, pxW, pxH, 0, 0, BlackPixel(dpy, scr));
    XStoreName(dpy, win, "Tap Zap");
    ...
    if (popover) evmask |= FocusChangeMask; // dismiss-on-focus-loss like a real popover
    XSelectInput(dpy, win, evmask);
    Atom wmDelete = XInternAtom(dpy, "WM_DELETE_WINDOW", False);
    XSetWMProtocols(dpy, win, &wmDelete, 1);
    XMapWindow(dpy, win);
    if (popover) { XMoveWindow(dpy, win, popoverX, popoverY); XRaiseWindow(dpy, win); }
    ...
    // Preserve the user's enabled preference from earlier isolated test builds.
    const char* privateConfig = std::getenv("XDG_CONFIG_HOME");
    const bool legacyLogin = privateConfig && ::access((std::string(privateConfig) + "/autostart/tapzap.desktop").c_str(), F_OK) == 0;
    if (st.launchAtLogin || (cfg.launchAtLogin && legacyLogin)) {
        st.loginError = !setLaunchAtLogin(true); // Also refresh the launcher after an app update.
        st.launchAtLogin = isLaunchAtLogin();
    }
    ...
    auto applyFilter = [&]() {
        ...
        std::string e;
        if (st.enabled) backend->setFilter(st.intensity, st.brightness, e);
        else backend->restore(e);
    };
    auto refreshFlicker = [&]() {
    ...
    const int ipcfd = ipcListen(); // on/off/toggle/show control socket
    ...
    pid_t trayPid = -1;
    {
        ...
        if (::access(traybin.c_str(), X_OK) == 0) {
            const pid_t p = fork();
            if (p == 0) { execl(traybin.c_str(), "tapzap-tray", static_cast<char*>(nullptr)); _exit(127); }
            if (p > 0) trayPid = p;
        }
    }
    const bool trayRunning = (trayPid > 0);

    auto handleX = [&](XEvent& ev) {
        // Coalesce bursts so a high-latency/remote X link doesn't queue one full-frame redraw per event.
        if (ev.type == MotionNotify) {
            XEvent nx;
            while (XCheckTypedWindowEvent(dpy, win, MotionNotify, &nx)) ev = nx;
        ...
        if (ev.type == Expose) {
            ...
            if ((Atom)ev.xclient.data.l[0] == wmDelete) {
                if (trayRunning) { XUnmapWindow(dpy, win); XFlush(dpy); } // hide to tray
                else running = false;
            }
        } else if (ev.type == FocusIn && popover) {
            focusSeen = true;
        } else if (ev.type == FocusOut && popover && trayRunning) {
            // popover dismiss: clicking away hides to the tray (tray toggle / Escape brings it back).
            // Only after the window has genuinely held focus (so it can't vanish at launch), and not
            // during a fader drag (a grab reports a spurious focus-out).
            if (focusSeen && ev.xfocus.mode == NotifyNormal && ev.xfocus.detail != NotifyInferior && dragging < 0) {
                XUnmapWindow(dpy, win); XFlush(dpy);
            }
        } else if (ev.type == KeyPress) {
            KeySym ks = XLookupKeysym(&ev.xkey, 0);
            if (ks == XK_Escape && st.view == 1) { st.view = 0; redraw(); }
            else if (ks == XK_Escape && popover && trayRunning) { XUnmapWindow(dpy, win); XFlush(dpy); } // dismiss
            ...
            if (frameless && lx >= W - 46 && ly < HEADER_H) {
                ...
                if (ly >= 104 - 20 && ly <= 104 + 20) {         // LAUNCH AT LOGIN
            ...
            if (close != st.closeHovered) {
                st.closeHovered = close;
                XDefineCursor(dpy, win, close ? handCursor : arrowCursor);
                redraw();
            }
    ...
    while (running) {
        ...
        if (ipcfd >= 0 && FD_ISSET(ipcfd, &fds)) { // on/off/toggle/show from `tapzap <cmd>`
            const std::string cmd = ipcAccept(ipcfd);
            ...
            else if (cmd == "hide") { XUnmapWindow(dpy, win); XFlush(dpy); }
