...
int liveWindow(double i0, double b0, bool e0) {
    ...
    auto desiredScale = [&]() {
        const double desktop = desktopUiScale(dpy, scr);
    ...
    if (!haveBackend && (std::getenv("TAPZAP_PLASMA_HELPER") || std::getenv("TAPZAP_WLR_ENABLE") || std::getenv("TAPZAP_GNOME_HELPER"))) return 2;
