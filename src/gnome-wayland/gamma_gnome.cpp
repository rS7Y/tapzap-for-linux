...
namespace tapzap {
class GnomeBackend final : public IGammaBackend {
    int socket_ = -1;
    pid_t child_ = -1;
    int outputs_ = 0;
    bool failed_ = false;
    bool receive(std::string& err) {
        ...
        failed_ = true;
        err = "Gnome color helper disconnected or timed out";
    ...
    bool request(const std::string& line, std::string& err) {
        if (failed_ || socket_ < 0) { err = "Restart TAP ZAP to reconnect its Gnome color helper"; return false; }
        const auto message = line + "\n";
        size_t sent = 0;
        while (sent < message.size()) {
            auto n = send(socket_, message.data() + sent, message.size() - sent, MSG_NOSIGNAL);
            if (n < 0 && errno == EINTR) continue;
            if (n <= 0) { failed_ = true; err = "Gnome color helper is unavailable"; return false; }
            sent += n;
        }
        return receive(err);
    }
public:
    ~GnomeBackend() override {
        std::string err;
    ...
    bool init(std::string& err) override {
        const char* script = std::getenv("TAPZAP_GNOME_HELPER");
        if (!script || script[0] != '/' || access(script, R_OK)) { err = "Gnome helper is missing"; return false; }
        int pair[2];
        ...
        close(pair[1]);
        if (child_ < 0) { close(pair[0]); err = "Cannot start Gnome helper"; return false; }
        socket_ = pair[0];
        return receive(err);
    }
    ...
    std::string name() const override { return "gnome-wayland-gamma"; }
    int outputCount() const override { return outputs_; }
};
std::unique_ptr<IGammaBackend> makeGnomeBackend() { return std::make_unique<GnomeBackend>(); }
}
