...
if [ "$(uname -s)" = "Linux" ]; then
  if [ "$HAVE_PC" = 1 ] && pkg-config --exists wayland-client && command -v wayland-scanner >/dev/null; then
    mkdir -p build
    wayland-scanner client-header protocols/wlr-gamma-control-unstable-v1.xml build/wlr-gamma-control-client.h
    wayland-scanner private-code protocols/wlr-gamma-control-unstable-v1.xml build/wlr-gamma-control.c
    cc -c -O2 $(pkg-config --cflags wayland-client) build/wlr-gamma-control.c -o build/wlr-gamma-control.o
    SRC="$SRC gamma_wayland.cpp build/wlr-gamma-control.o"
    FLAGS="$FLAGS -DHAVE_WLR_GAMMA $(pkg-config --cflags wayland-client)"
    LIBS="$LIBS $(pkg-config --libs wayland-client)"
    echo "[build] + native wlr Wayland gamma control"
  fi
  ...
  if [ "$HAVE_PC" = 1 ] && pkg-config --exists xrandr x11 xext xfixes; then
    SRC="$SRC gamma_xrandr.cpp gamma_plasma.cpp gamma_gnome.cpp"
